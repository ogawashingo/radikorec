...
現在の `src/lib/radiko.ts` では、取得したURLリストから **`dr-wowza` を含むURLを優先的に選択** するように実装されています。
今回の調査により、`dr-wowza` が最も汎用的かつ安定した配信元であることが裏付けられたため、この実装方針は適切であると結論付けられました。
...
- [radikoオーディオアド（詳細情報）](https://radiko.jp/rg/ad/)
